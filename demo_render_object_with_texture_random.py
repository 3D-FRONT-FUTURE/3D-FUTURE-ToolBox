'''
Render multi-view images for object with texture under random postion and random views
...
def render_function(model_file, texture_file):
    ...
    for i in range(args.views):
        ...
        cam_loc_z = random.uniform(0.8, dist*0.8)
        cam_loc_x = random.uniform(0, math.sqrt(dist**2 - cam_loc_z**2))
        cam_loc_y = math.sqrt(dist**2 - cam_loc_x**2 - cam_loc_z**2)
        cam.location = (cam_loc_x, cam_loc_y, cam_loc_z)
        b_empty.rotation_euler[2] = radians(random.uniform(0,360))
        
        scene.render.filepath = os.path.join(save_dir, 'image_' + '{0:03d}'.format(i))                              # rgb
        depth_file_output.file_slots[0].path = os.path.join(save_dir, 'depth_' + '{0:03d}'.format(i) + '_')         # depth
        normal_file_output.file_slots[0].path = os.path.join(save_dir, 'normal_' + '{0:03d}'.format(i) + '_')       # normal
        albedo_file_output.file_slots[0].path = os.path.join(save_dir, 'mask_' + '{0:03d}'.format(i) + '_')         # mask
